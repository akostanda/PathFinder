#include "../inc/pathfinder.h"

static unsigned int **copy_matrix(const char *file, unsigned int **matrix) {
    unsigned int **newm = NULL;
    int width = mx_matrix_width(file);

    newm = (unsigned int **)malloc(sizeof(unsigned int *) * 3);
    for (int i = 0; i < 3; i++) {
        newm[i] = (unsigned int *)malloc(sizeof(unsigned int) * width);
        for (int j = 0; j < width; j++) {
            newm[i][j] = matrix[i][j];
        }
    }
    return newm;
}

static void min_ways_cycle(const char *file, unsigned int **minwaymat, t_ints *n) {
    int **matrix = mx_matrix_filling(file);
    int width = mx_matrix_width(file);

    for (int i = 0; i < width; i++) {
        if (minwaymat[2][i] != 1 && matrix[(n->pivot)][i] != MAX_INT
            && (matrix[(n->pivot)][i] + minwaymat[0][(n->pivot)] < minwaymat[0][i])) {   
            minwaymat[0][i] = matrix[(n->pivot)][i] + minwaymat[0][(n->pivot)];
            minwaymat[1][i] = (n->pivot);
        }
    }
    mx_del_intarr(&matrix, width);
}

static void parallel_ways_cycle(const char *file, unsigned int **minwaymat, t_ints *n, t_minways **list) {
    int **matrix = mx_matrix_filling(file);
    int width = mx_matrix_width(file);
    unsigned int **copy = NULL;

    for (int i = 0; i < width; i++) {
        if (minwaymat[2][i] != 1 && matrix[(n->pivot)][i] != MAX_INT
            && (matrix[(n->pivot)][i] + minwaymat[0][(n->pivot)] == minwaymat[0][i])) {
            copy = copy_matrix(file, minwaymat);
            copy[1][i] = (n->pivot);
            mx_allmin_ways(file, copy, n, list);
            mx_del_uarr(&copy, 3);
        }
    }
    mx_del_intarr(&matrix, width);
}

void mx_allmin_ways(const char *file, unsigned int **minwaymat, t_ints *n, t_minways **list) {

    while (!(mx_ways_stopper(file, minwaymat))) {
        mx_min_ways(file, minwaymat, n->index, &n->pivot);
        min_ways_cycle(file, minwaymat, n);
        parallel_ways_cycle(file, minwaymat, n, list);
    }
    mx_push_filter(file, list, minwaymat);
}
